...
def plot_halving_curves(curves:dict,anchors:dict):
    """
    Plots the learning curves for each dataset. Saves the results as 
    'halving.png' in the current folder.
    Args:
        curves (dict): dictionary containing dataset names as keys and their
        learning curves in a list as values.
        anchors (dict): dictionary containing dataset names as keys and the 
        anchors at which they are evaluated in a list as values.
...
class SuccessiveHalving():
    """Class to perform Successive Halving. At initialization the hyperparameter
    space has to be defined.
    Initialize gives the class the number of iterations and the halving rate, with 
    this the initial and consectutive budgets are calculated.
    Run exectues the algorithm.
    """    
    def __init__(self, config_space,df):
        """
        Initiates successive Halving
        """
    ...
    def initialize(self,iterations = 7, halving_rate = 3):
        """Initizlize by defining the iterations and halving rate.
        Initial budget is halving_rate^iterations. Default is: 7^3=2187.

        Args:
            iterations (int, optional): Number of anchors at which to evaluate. 
            Defaults to 7.
            halving_rate (int, optional): Factor with which to reduce the number of 
            algorithms that are kept at each iteration. Defaults to 3.
        """        
